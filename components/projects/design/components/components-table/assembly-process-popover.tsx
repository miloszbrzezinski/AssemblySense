...
export const AssemblyProcessPopover = ({
  ...
  useEffect(() => {
    if (projectComponent.assemblyProcess) {
      setSelectedValue(
        `${projectComponent.assemblyProcess.processId} ${projectComponent.assemblyProcess.name}`,
      );
    }
  }, [projectComponent.assemblyProcess]);
  ...
  return (
    <Popover>
      <PopoverTrigger className="text-base h-10 w-full flex items-center justify-start hover:bg-slate-200">
        <h3 className="text-sm font-light px-2 whitespace-nowrap">{selectedValue}</h3>
      </PopoverTrigger>
