...
import { removeProjectTarget } from "@/actions/project-target";

export const RemoveProjectTargetModal = () => {
  const { isOpen, onClose, type, data } = useModal();
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);

  const isModalOpen = isOpen && type === "removeProjectTarget";
  const { profileId, workspaceId, projectId, projectTargetId } = data;

  if (!profileId || !workspaceId || !projectId || !projectTargetId) {
    return;
  }
    ...
    <Dialog open={isModalOpen} onOpenChange={onClose}>
      <DialogContent className="bg-white text-black dark:bg-neutral-950 dark:text-gray-100 overflow-hidden p-0 gap-0">
        ...
        <DialogDescription className="text-zinc-500 dark:text-neutral-400 p-3">
          ...
          Target will be permanently removed.
        </DialogDescription>
