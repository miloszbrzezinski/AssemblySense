...
interface ComponentEventSymbolProps {
  profileId: string;
  workspaceId: string;
...
export const ComponentEventSymbol = ({
  profileId,
  workspaceId,
  projectComponent,
  componentEvent,
}: ComponentEventSymbolProps) => {
  ...
  useEffect(() => {
    if (componentEvent.addressIO?.symbol) {
      setComponentName(componentEvent.addressIO?.symbol);
    }
  }, [componentEvent]);
